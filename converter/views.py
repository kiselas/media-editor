...
from compressor.forms import ConvertImageForm, ConvertVideoForm, ConvertVideoToGifForm
from compressor.utils import get_file_format
from converter.constants import DefaultContext
from converter.tasks import convert_image_file, convert_video_file, convert_video_to_gif
...
def convert_video(request):
    context = DefaultContext.CONVERT_VIDEO
...
def convert_image(request):
    context = DefaultContext.CONVERT_IMAGE
    if request.method == "POST":
...
def convert_video_to_gif_view(request):
    context = DefaultContext.CONVERT_VIDEO_TO_GIF
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if file_format in AVAILABLE_VIDEO_FORMATS and file_from_request.size < MAX_VIDEO_SIZE:
                ...
                convert_video_to_gif.delay(file_path, file_identifier, file_format, start_time, end_time,
                                           quantize_algorithm)
